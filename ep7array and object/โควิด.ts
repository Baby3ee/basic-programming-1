...
const deathmoreFiveDatabase = database.filter(function(element,index){ //filter เป็นการทำคัดกรองสมาชิกตามเงื่อนไขที่เรากำหนดไว้
    return element.total_death > 5 && element.province !== 'ทั้งประเทศ' //เป็นการที่กำหนดว่ามีคนตายมากกว่า 5 คน และไม่เอาทั้งประเทศ
}).map(function(element , index){ // map ทำหน้าที่สร้าง array ใหม่ value ตามเงื่อนไขที่เรากำหนด
    return ({
        province: element.province,
        total_death : element.total_death
    })
}).filter(function(element , index){
    return element.province.includes('นคร') //เป็นการที่เราสร้างเงื่อนไขว่าต้องการจังหวัดที่มีนคร
})
